...
const createproduct = () => {
  ...
  return (
    <div>
      <form className="absolute top-1/2 left-1/2 transform -translate-x-1/2 -translate-y-1/2 shadow-2xl w-1/2 " onSubmit={handleSubmit}>
        <div className="w-full p-10 space-y-5">
          <div className="space-y-5">
            <label className="font-normal text-lg text-gray-700">Blog Title</label>
            <input
          ...
          <ImgCrop rotate aspect={9 / 3}>
            <Upload multiple={false} customRequest={dummyRequest} listType="picture-card" onChange={handleImage} onPreview={onPreview}>
              {fileList.length < 5 && "+ Upload"}
            </Upload>
          </ImgCrop>

          <div className="space-y-5">
            <label className="font-normal text-lg text-gray-700 ">Description</label>
            <textarea
              className="w-full banner py-3 px-2 rounded-sm outline-none text-gray-700 font-semibold tracking-wide"
              onChange={(e) => {
